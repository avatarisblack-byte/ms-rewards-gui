...
| `gui/lib/logger.js` | parseLogLine / listLogFiles / readLogFile（2026-08-18 新增）；parseLogLine 双格式（2026-09-06）：v3 双时间戳行（UTC-ISO 开头）原路解析，v4 单时间戳行（[本地时间] 开头，V4-china Logger 仅打 toLocaleString）utcTime=null 由文件名兜底日期归属，v4 行此前被整行丢弃致统计归零；parseLogLine 解析前剥离行尾 `\r`（JS 正则的 `.` 不匹配 `\r`，CRLF 日志会被整行丢弃导致统计归零），readLogFile 以 `YYYY-MM-DD` 白名单校验并自动补 `.log` 后缀（原先按日期查询恒返回空，且 dateStr 可穿越目录）（2026-08-20 修复） |
