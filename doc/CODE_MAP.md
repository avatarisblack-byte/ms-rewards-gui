...
| `gui/lib/logger.js` | parseLogLine / listLogFiles / readLogFile（2026-08-18 新增）；parseLogLine 多格式（2026-09-06，v4 文件行为第四种：行首 formatLocalTimestamp 本地毫秒时间戳+双括号，账户在第2个括号）：v3 双时间戳行（UTC-ISO 开头）原路解析，v4 单时间戳行（[本地时间] 开头，V4-china Logger 仅打 toLocaleString）utcTime=null 由文件名兜底日期归属，v4 行此前被整行丢弃致统计归零；parseLogLine 解析前剥离行尾 `\r`（JS 正则的 `.` 不匹配 `\r`，CRLF 日志会被整行丢弃导致统计归零），readLogFile 以 `YYYY-MM-DD` 白名单校验并自动补 `.log` 后缀（原先按日期查询恒返回空，且 dateStr 可穿越目录）（2026-08-20 修复） |
| `gui/lib/summary.js` | summarizeLogs / summarizeAllLogs / generateSummary / writeSummaryFile（2026-09-06 v4 双格式：parseAccountEndFields 兼容「总计:+N|原始:X→新值:Y」与「获得积分=N|原余额|现余额」，activityGain 活动兜底事件白名单+双键（获得积分=|pointsGained=）且 SEARCH-BING 限定单次行防汇总双计，主进程过滤「主进程」/「MAIN」双值）；收益口径：ACCOUNT-END 累加 + 本地时区日期键 + todayTotal（2026-08-18/19 新增）；summarizeLogs 取 `e.message \|\| ''` 后再匹配，残缺条目不再抛 TypeError（2026-08-20 加固）；generateSummary 按「运行段」聚合（2026-08-22 修复）：ACCOUNT-START 为段边界，段内有 ACCOUNT-END 用 END 权威总计、无 END 用段内活动积分兜底，段收益归属段结束日——修复同一天多次运行（脚本重启/中断续跑）时仅取最后一次 ACCOUNT-END、丢弃被截断运行收益的漏算（实测 guidata 漏 255 分，grandTotal 2050→2305）；新增 run 级事件黑名单 `RUN_LEVEL_EVENTS` + `isRunLevelEvent`，`summarizeLogs`/`generateSummary` 双处按 event 跳过（2026-08-23 修复）：上游 RUN-END 行 [账户] 字段被动态 userName 污染成末尾账号名，仅按 account 过滤会覆盖末尾账号 lastEvent 并在跨天运行时污染收益段归属；2026-09-07 补入 v4 的 ACCOUNT-DELAY（账号间延迟事件挂在前一账号名下，覆盖其 lastEvent=ACCOUNT-END 致完成指示丢失，实测仅末账号幸存） |
...
| `gui/lib/routes/sessions.js` | Session zip 导入/导出（防穿越 + .bak；2026-09-06 起经 lib/sessionFiles.js 双格式：v4 sessions/sessions.db+WAL 优先、v3 session_*.json 回退；移除导出残留 CORS *） |
| `gui/lib/routes/data.js` | 一键数据导入/导出（sessions+logs+accounts.json+config.json 打包恢复；sessions 环节 2026-09-06 起经 lib/sessionFiles.js 双格式支持 v4 sessions.db） |
...
| `gui/lib/routes/system.js` | 系统：POST `/api/shutdown`、GET `/api/stats`/`/api/summary`、GET `/api/keepalive`（SSE 保活）、POST `/api/setup`、GET `/api/setup/status`（2026-09-06 新增：环境就绪实时检测 deps/browser/build/envFile，前端据此把「安装环境」按钮切换为「✓ 环境已安装」绿色态并打开详情弹窗）（2026-08-18 新增）；三个读接口校验 req.method 返回 405（2026-08-20 加固）；`/api/setup` spawn `setup.bat` 前注入剔除 `allow-scripts` 的干净 `NPM_CONFIG_USERCONFIG`，规避用户级 `.npmrc` 的 allow-scripts 在 npm 11.17 嵌套安装下的 EALLOWSCRIPTS，不改动上游非 GUI 文件（2026-08-21）；**服务常驻改造（2026-08-23）**——删除「所有保活连接断开后 5s 静默期自杀（process.exit）」逻辑，keepalive 断开仅记日志「所有页面已断开保活连接，服务继续常驻运行」，停止方式仅剩 `/api/shutdown`/`stop-gui.bat`/Ctrl+C（根因：Edge 睡眠标签页冻结后台页面使 SSE 断开且页面无法重连，静默期走完服务被"清掉"且静默模式无感知） |
| `gui/start-gui.bat` | 一键启动（常规模式）：**纯 ASCII（无中文，避免任何代码页乱码）**：`cd /d %~dp0` → node 读取 `gui-settings.json` 端口注入 `PORT`（失败回退 3000）→ 校验 server.js → ping 延迟 ~1s → **Edge/Chrome `--app` 模式弹独立应用窗口**（无标签栏/地址栏、任务栏独立图标，1400x900；三级回退至默认浏览器普通标签）（**无 PowerShell**）→ 当前窗口前台跑 `node server.js`（日志窗口） |
...
| 日期 | 内容 |
|------|------|
| 2026-09-06 | **v4 文件日志行格式适配**：parseLogLine 新增 v4 文件行分支（formatLocalTimestamp 前缀），修复账号卡片/收益统计在 v4 运行下停更；newlog 1077 行实测全解析，详见 `gui/CHANGELOG.md` |
